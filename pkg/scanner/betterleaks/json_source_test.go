...
func TestJSON(t *testing.T) {
	...
	err := jsonData.Fragments(context.Background(), func(fragment sources.Fragment, err error) error {
	...
	for _, fragment := range fragments {
		assert.Contains(t, expected, fragment.Attributes[sources.AttrPath])
		assert.Equal(t, expected[fragment.Attributes[sources.AttrPath]], fragment.Raw, "path=%s", fragment.Attributes[sources.AttrPath])
	}
}
