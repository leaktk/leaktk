...
func TestContainerImage(t *testing.T) {
	...
	t.Run("FragmentsNormal", func(t *testing.T) {
		...
		assert.Equal(t, "Fake Leaks", fragments[0].Attributes[AttrOCIImageAuthorName])
		assert.Equal(t, "fake-leaks@leaktk.org", fragments[0].Attributes[AttrOCIImageAuthorEmail])
	})
