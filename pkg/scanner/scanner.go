	...
	s.scanQueue.Recv(func(msg *queue.Message[*proto.Request]) {
		...
		case proto.GitRepoRequestKind:
			...
			if !request.Opts.Local {
				if sourcePath, gitDir, err = s.cloneGitRepo(request.Resource, request.Opts); err != nil {
...
func (s *Scanner) cloneGitRepo(cloneURL string, opts proto.Opts) (string, string, error) {
