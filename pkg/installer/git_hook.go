...
func findGitDirs(ctx context.Context, root string) ([]string, error) {
	...
	err := filepath.WalkDir(root, func(path string, d iofs.DirEntry, err error) error {
		...
		if d.Name() != ".git" {
			logger.Debug("skipping non-git path: path=%q", path)
			return nil
		}

		repoInfo, err := git.GetRepoInfo(path)
		if err != nil {
...
func gitUserTemplateDir(ctx context.Context) (string, error) {
	...
	if err := os.MkdirAll(defaultDir, 0700); err != nil {
		return "", fmt.Errorf("could not create template dir: %w path=%q", err, templateDir)
	}

	if err := git.SetGlobalConfigPath("init.templateDir", templateDir); err != nil {
		return "", fmt.Errorf("could not set template dir: %w path=%q", err, templateDir)
	}

	logger.Info("configured git init.templateDir: path=%q", templateDir)
	return templateDir, nil
}

// gitCreateHookScript installs a git hook script into installDir (the .git directory).
func gitCreateHookScript(hook Hook, installDir string, force bool) error {
	hooksDir := filepath.Join(installDir, "hooks")
	hookPath := filepath.Join(hooksDir, hook.Event())

	if fs.IsExecutable(hookPath) && !force {
		logger.Info("skipping existing hook: force install not enabled: path=%q force_install=%v", hookPath)
		return nil
	}

	if err := os.MkdirAll(hooksDir, 0700); err != nil {
		return fmt.Errorf("could not create hooks dir: %w path=%q error=%w", err, hooksDir)
	}
...
func GitHookInstall(ctx context.Context, cfg *config.Config, opts GitHookOpts) error {
	...
	if opts.Path != "" {
		...
		for _, gitDir := range gitDirs {
			logger.Info("installing hook in git dir: hookname=%q path=%q", hookname, userGitTemplateDir)
			if err := gitCreateHookScript(opts.Hook, gitDir, opts.Force); err != nil {
				logger.Info("error installing hook in git dir: %v hookname=%q path=%q", err, hookname, userGitTemplateDir)
				hadErrors = true
			}
		}
	}
	...
	if opts.UserTemplateDir {
		...
		if err != nil {
			...
			logger.Info("installing hook in user template dir: hookname=%q path=%q", hookname, userGitTemplateDir)
			if err := gitCreateHookScript(opts.Hook, userGitTemplateDir, opts.Force); err != nil {
				logger.Info(
					"error installing hook in user template dir: %v hookname=%q path=%q",
	...
	if opts.SystemTemplateDir {
		logger.Info("installing hook in system template dir: hookname=%q path=%q", hookname, systemGitTemplateDir)
		if err := gitCreateHookScript(opts.Hook, systemGitTemplateDir, opts.Force); err != nil {
			logger.Info(
				"error installing hook in system template dir: %v hookname=%q path=%q",
